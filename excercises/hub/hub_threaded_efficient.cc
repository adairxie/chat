...
{
 public:
  typedef std::map<std::string, Topic> TopicList;
  typedef boost::shared_ptr<TopicList> TopicListPtr;

  PubSubServer(EventLoop* loop, const InetAddress& listenAddr)
    : server_(loop, listenAddr, "PubSubSerber"),
      topics_(new TopicList)
  {
...
//    loop_->runEvery(1.0, boost::bind(&PubSubServer::timePublish, this));
 ...
 private:
  ...
  Topic& getTopic(const string& topic)
  {
    ...
    if (topics_.unique())
    {
      topics_.reset(new TopicList(*topics_));
    }
    assert(topics_.unique());
